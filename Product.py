...
while True: #Gathering the product code. Using a while loop and valueErrors to make sure that the input received is not letters or below 100 or above 1000
    try:
        product_Code = int(input('What is the code of the product?  '))
...
product_Name = str(input("What is the name of the product? ")) # No need for the loops since we only need string
if (product_Name == ""): # If they enter nothing, it should make the product name 'Unnamed Product'
...
while True: #Gathering the sales price. Using a while loop and valueErrors to make sure that the input received is not letters or below 0 
    try:
        product_salesPrice = float(input('What is the retail price of the product?  '))
        if (product_salesPrice < 0):
            raise ValueError       
        break
    except ValueError:
...
while True: #Gathering the manufacture cost. Using a while loop and valueErrors to make sure that the input received is not letters or below 0 
    try:
        product_manufactureCost = float(input('What is the manufacture price of the product?  '))
        if (product_manufactureCost < 0):
            raise ValueError       
        break
    except ValueError:
...
while True: #Gathering the stock level. Using a while loop and valueErrors to make sure that the input received is not letters or below 0 
    try:
        stock_Level = int(input('How much stocks of your product do you have right now?  '))
        if (stock_Level < 0):
            raise ValueError       
        break
    except ValueError:
...
p = Product(product_Code,product_Name,product_salesPrice,product_manufactureCost,stock_Level,estimated_Monthly_UnitsManufactured,months,0,0)
